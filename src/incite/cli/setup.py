...
Orchestrates the multi-step first-run setup flow: source selection, paper
loading, index building (local) or cloud processing, and a test query.
Source-specific interactive steps are delegated to ``cli/setup_sources.py``.

Related modules:
    - incite.cli.setup_sources: Per-source interactive configuration steps.
    - incite.webapp.state: Config persistence and retriever construction.
"""
...
from incite.cli.setup_sources import (
    _setup_folder,
    _setup_interactive,
