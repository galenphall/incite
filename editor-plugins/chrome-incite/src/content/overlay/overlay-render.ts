...
export function renderRail(): string {
  return `<div class="incite-rail">
    ...
    <button class="rail-btn" data-action="hide" aria-label="Hide overlay" title="Hide overlay (re-enable in InCite settings)">
      ${ICON_HIDE}
    </button>
  </div>`;
}
...
function renderPopupEmpty(): string {
  return `<div class="popup-empty">
    ...
    <p>Try selecting a different passage, or expand your collection.</p>
  </div>`;
}
