 ...
 */
import { InCiteClient, extractContext } from "@incite/shared";
import type { ExtractedContext } from "@incite/shared";
...
/**
 * Extract citation context around the cursor in Google Docs when no text is selected.
 *
 * Uses the Zotero-style placeholder approach to determine cursor position:
 * 1. Insert a unique placeholder at the cursor via content script paste event
 * 2. Fetch document via REST API and find the placeholder
 * 3. Delete the placeholder via REST API
 * 4. Extract surrounding sentences using extractContext()
 */
export async function getGDocsCursorContextForRecs(
  tab: chrome.tabs.Tab,
  ...
  try {
    const placeholder = `\u27E6INCITE-CTX-${crypto.randomUUID().slice(0, 8)}\u27E7`;

    // Ask content script to paste placeholder at cursor
    ...
    // Fetch document via REST API and find the placeholder
    const doc = await gdocsApi.getDocument(docId);
    const fullText = extractFullText(doc);
    const pos = fullText.indexOf(placeholder);
    if (pos === -1) return null;

    // Delete the placeholder via REST API
    const docIndex = textOffsetToDocIndex(doc, pos);
    await gdocsApi.batchUpdate(docId, [
      { deleteContentRange: { range: { startIndex: docIndex, endIndex: docIndex + placeholder.length } } },
    ]);

    // Extract context from the clean text (without placeholder)
    const cleanText = fullText.slice(0, pos) + fullText.slice(pos + placeholder.length);
    return extractContext(cleanText, pos, contextSentences);
  } catch {
    return null;
  }
}
