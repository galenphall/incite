...
const showOverlayRail = document.getElementById("showOverlayRail") as HTMLInputElement;
...
// Overlay rail visibility lives in chrome.storage.sync (not in the typed
// settings object) because the overlay content script reads it directly
// without needing the full settings round-trip. Default: off — opt-in to
// avoid covering Google Docs's right sidebar buttons or Overleaf's PDF pane.
chrome.storage.sync.get("incite_overlay_visible", (result) => {
  showOverlayRail.checked = result.incite_overlay_visible === true;
});

showOverlayRail.addEventListener("change", () => {
  chrome.storage.sync.set({ incite_overlay_visible: showOverlayRail.checked });
});
